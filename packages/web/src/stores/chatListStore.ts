...
import {
  ...
  isUnread,
  type StreamWithLast,
  type StreamEntryInternal,
  type DmEntryInternal,
} from "../components/ui/Sidebar/data";
import type { SidebarChat } from "../components/ui/Sidebar/types";
...
function dmsMapToSortedDms(map: Map<string, DmEntryInternal>): Extract<SidebarChat, { type: "dm" }>[] {
...
function mergeStreamEntry(
  ...
  const topicEntry = { subject: topicSubject, lastMessage: topicLastMessage, time: topicTime, ts: topicTs, unreadCount, lastMessageId };
  if (!existing) {
    const topics = new Map<string, { subject: string; lastMessage: string; time: string; ts: number; unreadCount: number }>();
...
interface ChatListState {
  ...
  /** Последние применённые сообщения — для пересборки сайдбара, когда currentUserId загрузится позже сообщений */
  lastAppliedMessages: ZulipRawMessage[] | null;
  /** message_id → location для учёта update_message_flags и delete_message */
...
export const useChatListStore = create<ChatListState>((set, get) => ({
  ...
  addMessage(message) {
      ...
      set((state) => {
        ...
        if (existing && message.timestamp <= existing.ts) {
          ...
          if (existingTopic && message.timestamp <= existingTopic.ts) {
            ...
            const nextTopics = new Map(existing.topics);
            nextTopics.set(topic.subject, { ...existingTopic, unreadCount: existingTopic.unreadCount + topicUnreadDelta });
        ...
        return { streamsMap: next, messageIdToLocation: new Map(state.messageIdToLocation).set(message.id, { type: "stream", stream_id, topic: topic.subject }) };
      });
      return;
    }
    ...
    if (type === "private" && Array.isArray(message.display_recipient)) {
      ...
      set((state) => {
        ...
        next.set(key, { ...dmEntry, unreadCount: (existing?.unreadCount ?? 0) + unreadDelta, avatar_url, lastMessageId: message.id });
  ...
  addMessages(messages) {
    const currentUserId = get().currentUserId;
    type StreamMax = { stream_id: number; m: ZulipRawMessage };
    type DmMax = { key: string; m: ZulipRawMessage };
    ...
    set((state) => {
      ...
      for (const m of dmByKey.values()) {
        ...
        nextDms = new Map(nextDms);
        const avatar_url = dmEntry.avatar_url ?? existing?.avatar_url;
        nextDms.set(key, { ...dmEntry, unreadCount: (existing?.unreadCount ?? 0) + unreadDelta, avatar_url, lastMessageId: m.id });
      }
  ...
  setCurrentUserId(id) {
    ...
    // Если me загрузился позже сообщений — сайдбар мог быть собран с currentUserId=null (неверные ключи DM и имена). Пересобираем.
    ...
    set((state) => {
      ...
      for (const mid of messageIds) {
        ...
        if (loc.type === "stream") {
          ...
          if (!stream) continue;
          const topic = stream.topics.get(loc.topic);
          if (!topic || topic.lastMessageId !== mid) continue;
          nextStreams = new Map(nextStreams);
          const nextTopics = new Map(stream.topics);
          ...
          const dm = nextDms.get(loc.dmKey);
          if (!dm || dm.lastMessageId !== mid) continue;
          nextDms = new Map(nextDms);
  ...
  streams() {
    return streamsMapToSortedStreams(get().streamsMap);
  },

  dms() {
    return dmsMapToSortedDms(get().dmsMap);
  },

  chatsSortedByLastMessage() {
    const { streamsMap, dmsMap } = get();
